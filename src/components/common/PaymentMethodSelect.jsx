...
import { ChevronDown, Check, Loader2, CreditCard, Building2, Smartphone, Wallet } from "lucide-react";
...
const getPaymentIcon = (type) => {
  ...
  if (upperType.includes("BANK") || upperType.includes("ACH") || upperType.includes("SEPA")) return Building2;
  if (upperType.includes("APPLE") || upperType.includes("GOOGLE") || upperType.includes("MOBILE")) return Smartphone;
...
export default function PaymentMethodSelect({
  ...
  const CurrentIcon = selectedMethod ? getPaymentIcon(getMethodType(selectedMethod)) : Wallet;
  ...
  return (
    <div className="relative z-20 w-full" ref={dropdownRef}>
      ...
      {/* Dropdown Trigger */}
      <div
        onClick={() => !loadingMethods && availableMethods?.length > 0 && setOpen((prev) => !prev)}
      ...
      >
        {loadingMethods ? (
          ...
          <span className="text-sm text-gray-400">No methods available</span>
        ) : currentMethodName ? (
          <>
          ...
          >
            <ul className="max-h-52 overflow-y-auto">
              {availableMethods.map((method, idx) => {
                ...
                const fee = method.fee !== undefined && method.fee !== null 
                  ? (typeof method.fee === "number" ? `${method.fee}%` : method.fee)
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <MethodIcon className="w-4 h-4 text-indigo-400" />
                      <div>
